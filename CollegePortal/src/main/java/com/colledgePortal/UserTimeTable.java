...
@WebServlet("/displayTimeTableImage")
public class UserTimeTable extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            pstmt = conn.prepareStatement("SELECT timeTableImage FROM TimeTable WHERE id = ?");
            pstmt.setInt(1, id);
            rs = pstmt.executeQuery();

            if (rs.next()) {
                Blob blob = rs.getBlob("timeTableImage");
                ...
                response.sendRedirect("userDashboard.jsp?id=" +1);
