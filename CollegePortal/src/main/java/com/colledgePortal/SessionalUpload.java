...
@WebServlet("/sessionalImage")
@MultipartConfig
public class SessionalUpload extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            pstmt = conn.prepareStatement("INSERT INTO Sessional (sessionalImage) VALUES (?)", Statement.RETURN_GENERATED_KEYS);
            if (inputStream != null) {
...
//                    response.sendRedirect(request.getContextPath() + "/displayImage?id=" + id);
                    response.sendRedirect("sessional.jsp?id=" + id);
                } else {
                    response.sendRedirect("sessional.jsp");
                }
            } else {
                response.sendRedirect("sessional.jsp");
            }
